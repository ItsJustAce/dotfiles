...
function load_config() {
    ...
    showOutputWindow = vscode.workspace.getConfiguration('pixelbyte.love2d').get('revealStatus');
    relSearchDir = vscode.workspace.getConfiguration('pixelbyte.love2d').get('srcDir').toString();
    if (firstConfigLoaded) {
        outChannel.appendLine("Reloading config (toggled states will be reset).");
        if (showOutputWindow)
            outChannel.show(true);
    }
    firstConfigLoaded = true;
}
let searchedWorkspace = false;
function run_love() {
    runLoveFlag = false;
    ...
    if (fs.existsSync(loveExe)) {
        if (fs.lstatSync(loveExe).isDirectory()) {
            vscode.window.showErrorMessage(`pixelbyte.love2d.path '${loveExe}' must be an executable file, not a directory. Check your settings.`);
        }
        else {
            do_run_love(args);
            return;
    ...
    if (!searchedWorkspace) {
        ...
        vscode.workspace.findFiles("**/love.exe", "**/node_modules/**", 1).then(uriArray => {
            if (uriArray[0]) {
                outChannel.append(`pixelbyte.love2d.path '${loveExe}' not valid but found '${uriArray[0].fsPath}' in the workspace. Trying that instead.`);
                loveExe = uriArray[0].fsPath;
                do_run_love(args);
            }
            else
                vscode.window.showErrorMessage(`Path specified in pixelbyte.love2d.path '${loveExe}' does not exist. Check your settings.`);
        }, () => {
            vscode.window.showErrorMessage(`Path specified in pixelbyte.love2d.path '${loveExe}' does not exist. Check your settings.`);
        });
    }
...
function activate(context) {
    ...
    // love2dOut.show(true);
    load_config();
    statusDebug = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 5);
    statusDebug.text = "Debug: " + (debugMode ? "True" : "False");
    statusDebug.command = "pixelbyte.love2d.debug.toggle";
    statusDebug.show();
    statusRunOnSave = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 4);
    statusRunOnSave.text = "RunOnSave: " + (runOnSave ? "True" : "False");
    statusRunOnSave.command = "pixelbyte.love2d.runOnsave.toggle";
    statusRunOnSave.show();
...
function deactivate() {
    for (var key in loveProcesses) {
        var pid = Number(key);
        if (isNaN(pid)) {
            vscode.window.showErrorMessage("Unable to convert pid to a number: " + pid);
